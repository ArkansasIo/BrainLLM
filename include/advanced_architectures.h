...
struct AirLLMRuntimeConfig {
    std::string python_executable = "python";
    std::string airllm_root = "third_party/airllm";
    std::string runner_script = "tools/airllm_runner.py";
    std::string model_id;
    std::string layer_shards_path;
...
public:
    AirLLMBridge();
    explicit AirLLMBridge(const AirLLMRuntimeConfig& config);
    ~AirLLMBridge() = default;

    void configure(const AirLLMRuntimeConfig& config);
    AirLLMRuntimeConfig get_config() const;

    bool is_source_available() const;
    std::string describe_runtime() const;
    std::string build_inference_command(const std::string& prompt,
                                        const std::string& output_path) const;

private:
    AirLLMRuntimeConfig config_;
