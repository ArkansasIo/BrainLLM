...
QuantumLLMIntegration::QuantumTextEncoding QuantumLLMIntegration::encode_text_quantum(
    ...
    QuantumTextEncoding encoding(num_qubits_);
...
QuantumLLMIntegration::QuantumSearchResult QuantumLLMIntegration::quantum_semantic_search(
    ...
    auto measured_bits = grovers_algorithm_->search(documents, query);
    result.matches.clear();
    for (int bit : measured_bits) {
        result.matches.push_back(bit ? query : "");
    }
...
QuantumLLMIntegration::QuantumFeatures QuantumLLMIntegration::extract_quantum_features(
    ...
    auto encoded = encode_classical_to_quantum(classical_features);
    features.quantum_features.reserve(encoded.size());
    for (float value : encoded) {
        features.quantum_features.emplace_back(value, 0.0f);
    }
