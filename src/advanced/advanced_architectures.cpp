...
// ========================================
// AIRLLM VENDORED RUNTIME BRIDGE
// ========================================

AirLLMBridge::AirLLMBridge() = default;

AirLLMBridge::AirLLMBridge(const AirLLMRuntimeConfig& config)
    : config_(config) {
}

void AirLLMBridge::configure(const AirLLMRuntimeConfig& config) {
    config_ = config;
}

AirLLMRuntimeConfig AirLLMBridge::get_config() const {
    return config_;
}

bool AirLLMBridge::is_source_available() const {
    const std::filesystem::path airllm_root(config_.airllm_root);
    const auto package_path = airllm_root / "air_llm" / "airllm";
    const auto init_path = package_path / "__init__.py";
    const std::filesystem::path runner_path(config_.runner_script);

    return std::filesystem::exists(init_path) &&
           std::filesystem::exists(runner_path);
}

std::string AirLLMBridge::describe_runtime() const {
    std::ostringstream description;
    description << "AIRLLM bridge: source="
                << (is_source_available() ? "available" : "missing")
                << ", root=" << config_.airllm_root
                << ", model="
                << (config_.model_id.empty() ? "<not configured>" : config_.model_id)
                << ", compression="
                << (config_.compression.empty() ? "none" : config_.compression)
                << ", max_new_tokens=" << config_.max_new_tokens;
    return description.str();
}

std::string AirLLMBridge::build_inference_command(const std::string& prompt,
                                                  const std::string& output_path) const {
    std::ostringstream command;
    command << quote_arg(config_.python_executable)
            << " " << quote_arg(config_.runner_script)
            << " --airllm-root " << quote_arg(config_.airllm_root)
            << " --model " << quote_arg(config_.model_id)
            << " --prompt " << quote_arg(prompt)
            << " --output " << quote_arg(output_path)
            << " --max-new-tokens " << config_.max_new_tokens;

    if (!config_.layer_shards_path.empty()) {
        command << " --layer-shards-path " << quote_arg(config_.layer_shards_path);
    }

    if (!config_.compression.empty()) {
        command << " --compression " << quote_arg(config_.compression);
    }
...
std::string AirLLMBridge::quote_arg(const std::string& value) {
    std::string escaped = "\"";
    for (char ch : value) {
        if (ch == '"') {
            escaped += "\\\"";
