...
void MainWindow::initialize_ui() {
    ...
    splitter->addWidget(editor_widget_);
    splitter->addWidget(chat_widget_);
    splitter->addWidget(console_widget_);
...
void MainWindow::on_process_button_clicked() {
    ...
    if (input.isEmpty()) {
        console_widget_->write_line("[Process] No input provided.");
        status_label_->setText("Status: Idle");
        return;
    }

    const auto response = QString::fromStdString(llm_engine_->process_input(input.toStdString()));
    console_widget_->write_line("[Input] " + input);
    console_widget_->write_line("[Process] " + response);
    status_label_->setText("Status: Processed");
}
...
void MainWindow::on_generate_button_clicked() {
    ...
    if (prompt.isEmpty()) {
        console_widget_->write_line("[Generate] No prompt provided.");
        status_label_->setText("Status: Idle");
        return;
    }

    const auto response = QString::fromStdString(llm_engine_->generate_response(prompt.toStdString(), 120));
    console_widget_->write_line("[Prompt] " + prompt);
    console_widget_->write_line("[Generate] " + response);
    status_label_->setText("Status: Generated");
}
...
void MainWindow::on_start_api_clicked() {
    if (!api_server_->is_running()) {
        ...
        if (api_server_->start()) {
            ...
            console_widget_->write_line("[API] Failed to start server on port 8080.");
        }
    } else {
        api_server_->stop();
...
void MainWindow::on_train_clicked() {
    ...
    if (!input.isEmpty()) {
        ...
        for (const auto& line : lines) {
            training_data.push_back(line.trimmed().toStdString());
        }
    }
    if (training_data.empty()) {
        training_data = {"Hello", "World", "AI", "Learning"};
    }
    llm_engine_->train(training_data);
    console_widget_->write_line(QString("[Train] Stored %1 training samples.").arg(static_cast<int>(training_data.size())));
    status_label_->setText("Status: Training Complete");
}

void MainWindow::on_reset_clicked() {
    llm_engine_->reset();
    console_widget_->write_line("[System] Brain state reset.");
    status_label_->setText("Status: Brain Reset");
}

void MainWindow::on_memory_clear_clicked() {
    llm_engine_->reset();
    console_widget_->write_line("[Memory] Cleared memory and context.");
